...
sat_data <- mutate(sat_data, z_spend_pct =as.numeric(scale(spend_pct)) )
...
We choose a bar chart to display the generated fake data with its mean highlighted which is about 980. The range of scores is reasonable with a wide range of data. but we don't have 1600? 
...
$z_{y_{i}\sim student-t(z_\nu, z_{\beta_{0}{+z_{\beta_{1}}*Spend_{i}+z_{\beta_{2}}*PrcntTake_{i}+z_{\beta_{3}}*Spend_{i}*PrcntTake_{i}, z_\sigma)$, where $i = 1,2,3,...,50$ indicating each subject; $z_\nu \sim e^{(1/30)}$, $z_\sigma \sim Normal(0,1)$, $z_{\beta_{0,1,2,3}}\sim student-t(10, \mu, \sigma)$ where $\mu\sim Normal(0,1)$, $\sigma\sim Normal(0,1)$. We choose 10 for $nu$ for $z_\beta$'s because we want theem to have some thickness to the tails but not too thick. We choose normal distributions for both $z_\beta$'s $\mu$ and $\sigma$ because although $z_\beta$'s are standardized, for practice purpose we still give them a hyper distribution.
